...
function TransactionModal({
    ...
    return (
        <dialog
            ref={dialogRef}
            onClose={onClose}
            className="m-auto w-full max-w-md rounded-lg border border-gray-700 bg-gray-900 p-6 text-white backdrop:bg-black/60 backdrop:backdrop-blur-md"
        >
            <form
                        ...
                        <td className="px-2 py-2 text-right text-sm font-semibold tabular-nums text-amber-300">
                            {isEditingKeyNumber ? (
                                <input
                                    ...
                                    onChange={(event) =>
                                        setKeyNumberDraft(event.target.value)
                                    }
